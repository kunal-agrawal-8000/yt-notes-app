...
/**
 * YouTube Study App (Local Only) - Enhanced with Playlist Support
 * - Paste & play YouTube links AND playlists (YouTube Iframe API, no DB)
 * - Individual video & playlist support with automatic video extraction
 ...
 *
 * ENHANCED: Full playlist support with API fallback and user guidance
 */

// ----------------------------- Helpers ---------------------------------
const LS_KEY = "yt_study_app_state_v1";
const YOUTUBE_API_KEY = "AIzaSyBC6U1_A4qg1o-RFzZHr-9xI9U3dl39TDs"; // Add your YouTube API key here for full playlist support
...
async function fetchPlaylistVideos(playlistId, sourceUrl = '') {
    ...
    // Fallback method: Use source URL if it contains a video ID
    try {
        ...
        // If no video ID in URL, return empty array for manual addition
        return [];
    } catch (error) {
...
export default function App() {
    ...
    const [isLoading, setIsLoading] = useState(false); // Loading state for playlist operations
    ...
    }, [getCurrentTime, seekTo, playPause]);
    ...
    // Enhanced handleAddVideo with playlist support
    async function handleAddVideo() {
        ...
        try {
            ...
            // Add videos to the current project
            setState((s) => ({
                ...s,
                projects: s.projects.map((p) =>
                    p.id === currentProject.id
        ...
        <div className="min-h-screen min-w-screen bg-neutral-50 text-neutral-900 dark:bg-neutral-900 dark:text-neutral-100 transition-colors">
            ...
            <div className={`max-w-8xl mx-auto px-1 sm:px-6 py-4 grid ${state.distractionFree ? "grid-cols-1" : "md:grid-cols-[2fr_1fr]"
                ...
                <div className="space-y-4">
                    ...
                    {/* Videos in Project - Enhanced with playlist index display */}
                    {!state.distractionFree && (
                        <div className="rounded-2xl p-3 border border-neutral-200 dark:border-neutral-800">
                            ...
                            <div className="grid sm:grid-cols-2 md:grid-cols-1 lg:grid-cols-2 gap-3">
                                {currentProject?.videos.map((v, index) => (
                                ...
                                {(!currentProject || currentProject.videos.length === 0) && (
                                    <div className="text-sm opacity-70">No videos yet. Paste a video or playlist link above and click Add.</div>
                                )}
                            </div>
                        </div>
                    )}
                ...
                {!state.distractionFree && (
                    <div className="space-y-4">
                        ...
                        <div className="rounded-2xl border border-neutral-200 dark:border-neutral-800 overflow-hidden" onDrop={onDropBookmark} onDragOver={allowDrop}>
                            <div className="p-3 border-b border-neutral-200 dark:border-neutral-800 flex flex-col items-center justify-between">
                                ...
                                <div className="flex items-center justify-between gap-4 mb-2">
                                    <input
                                        ...
                                        placeholder="Title"
                                        className="px-2 py-2 max-w-[7rem] rounded-xl bg-neutral-100 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700"
                                    />
                                    <input
                                        value={bookmarkTag}
                                        onChange={(e) => setBookmarkTag(e.target.value)}
                                        placeholder="Tag (e.g. formula)"
                                        className="px-3 py-2 max-w-[7rem] rounded-xl bg-neutral-100 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700"
                                    />
